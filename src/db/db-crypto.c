...
static const char *const csv_col_names[] = {
    [CRYPTO_CSV_COL_TS] = "timestamp",    [CRYPTO_CSV_COL_CLOSE] = "price",     [CRYPTO_CSV_COL_VOLUME] = "volume",
                                     ...
                                     void *priv_data)
{
    ...
    csv_item_t items[] = {
        ...
        [CRYPTO_CSV_COL_CLOSE] = { csv_parse_float, &crypto.close },
...
db_err_t db_crypto_import_csv(const char *csv_path, const char *sym_name)
{
    ...
    log_info("Imported %u crypto rows for symbol '%s'", ctx.line_count, sym_name);
    return db_txn_commit();
}
...
static csv_gen_err_t csv_gen_row(const csv_gen_ctx_t *gctx, void *priv_data)
{
    ...
    csv_gen_item_t items[] = {
        ...
        [CRYPTO_CSV_COL_CLOSE] = CSV_GEN_FLOAT(crypto.close),
