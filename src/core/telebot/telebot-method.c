...
static telebot_err_t telebot_post(const char *token, const char *method, const chttp_mime_gen_t *mimes,
                                  uint32_t mime_count, chttp_resp_cb_t http_cb, telebot_resp_cb_t telebot_cb)
{
    char url[URL_SIZE];
                                  ...
                                  telebot_resp_cb_t cb)
{
    ...
    return telebot_post(token, "getUpdates", mimes, mimes_count, telebot_get_updates_cb, cb);
}
