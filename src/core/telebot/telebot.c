...
static void parse_entities(const telebot_update_t *update)
{
    ...
    const telebot_message_t *msg = &update->content.message;
...
static void get_updates_cb(const telebot_resp_t *resp)
{
    ...
    telebot_get_updates(bot.token, last_update_id, TELEBOT_MAX_UPDATES, bot.upd_sec, NULL, 0, get_updates_cb);
}
                           ...
                           uint32_t handlers_count)
{
    ...
    return telebot_get_updates(token, 0, TELEBOT_MAX_UPDATES, upd_sec, NULL, 0, get_updates_cb);
}
